...
{	
	...
	{
		printf("******************************************************************\n");
		printf("******              Usage serial_filter_utils               ******\n");
		printf("******************************************************************\n");
		printf("* ./serial_filter_utils [dev_tty_name] <filter_word> <save2file> *\n");
		printf("******************************************************************\n");
		return 0;
	}

	
	fd = uart_open(fd, argv[1]);//选择的是ttsY1串口
	if(fd == -1)
	{
	...
	while(1){
		...
		if(argv[2] != NULL)
		{
			...
			{	
				if(r_buf[analysis_data_index] == argv[2][0])
				{
					if((ret-analysis_data_index) > strlen(argv[2]))
					{
						if(strncmp(r_buf+analysis_data_index, argv[2], strlen(argv[2])) == 0)
						{
